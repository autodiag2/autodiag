...
def test_simple():
    ...
    assert b.cmp(Buffer.from_ascii_hex("01020304"))
    b.append_str("Hello")
    assert b.cmp(Buffer.from_ascii("\x01\x02\x03\x04Hello"))
    b2 = Buffer.from_ascii_hex("48656c6c6f")
    b.prepend(b2)
    assert b.cmp(Buffer.from_ascii("\x48\x65\x6c\x6c\x6f\x01\x02\x03\x04Hello"))
