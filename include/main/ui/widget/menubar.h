...
static void menubar_data_source_filter_by_on_filter_check_toggled(GtkCheckMenuItem *check_item, gpointer user_data){ \
...
static GtkWidget* menubar_data_source_filter_by_find_widget_for_ecu(GtkWidget *menu, ECU *ecu){ \
...
static void menubar_data_source_filter_by_filter_change(const char *type,Buffer *address){ \
    if(strcmp(type,"clear")==0){ \
        ...
        if(ecu==null){ \
            ...
            GtkWidget *w=menubar_data_source_filter_by_find_widget_for_ecu(gui->menuBar.data.source.filter_by_menu,ecu); \
            if(w==null){ \
...
static void menubar_data_source_filter_by_register(const ECU *ecu){ \
    char *displayLabel; \
    asprintf(&displayLabel,"%s (%s)",ecu->name,buffer_to_hex_string(ecu->address)); \
    GtkWidget *filter_check=menubar_data_source_filter_by_find_widget_for_ecu(gui->menuBar.data.source.filter_by_menu,ecu); \
    if(filter_check==null){ \
        ...
        g_signal_connect(filter_check,"toggled",G_CALLBACK(menubar_data_source_filter_by_on_filter_check_toggled),gui->menuBar.data.source.all); \
