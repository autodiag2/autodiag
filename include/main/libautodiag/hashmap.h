...
#define HASHMAP_SRC_NEW(key_type, value_type) HASHMAP_H_NEW(key_type, value_type) { \
    hashmap_##key_type##_##value_type * hm = (hashmap_##key_type##_##value_type *)malloc(sizeof(hashmap_##key_type##_##value_type * hm)); \
    hm->size = 0; \
    hm->keys = null; \
    hm->values = null; \
    return hm; \
}
#define HASHMAP_H_FREE(key_type, value_type) hashmap_##key_type##_##value_type##_free(hashmap_##key_type##_##value_type *hm)
...
#define HASHMAP_H_GET(key_type, value_type) value_type * hashmap_##key_type##_##value_type##_set(hashmap_##key_type##_##value_type * hm, key_type *key)
#define HASHMAP_SRC_GET(key_type, value_type) HASHMAP_H_SET(key_type, value_type) { \
    assert(hm != null); \
    for(unsigned int i = 0; i < hm->size; i++) { \
...
#define HASHMAP_H_SET(key_type, value_type) bool hashmap_##key_type##_##value_type##_set(hashmap_##key_type##_##value_type * hm, key_type *key, value_type *value)
