...
static void recovery_mode() {
    ...
    if ( config.com.device.location != null ) {
        device_table_add_if_not_in_by_location(config.ephemere.device_table, config.com.device.location, AD_DEVICE_TYPE_AUTO);
...
static void launch_simulation_internal() {
    ...
    if ( gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gui->simulator.replay.enabled)) ) {
        ...
        for (GList *iter = rows; iter != null; iter = iter->next) {
            if ( iter == rows ) {
                ad_list_SimECU_empty(elm327->ecus);
            }
            ...
            g_list_free(children);
            ad_list_SimECU_append(elm327->ecus,ecu);
        }
        g_list_free(rows);
    }
