...
gboolean counter_draw_callback (GtkWidget *widget, cairo_t *cr, gpointer data) {
    ...
    char * img_path;
    final char * data_dir = config_get_data_directory_safe();
    asprintf(&img_path, "%s" PATH_FOLDER_DELIM "media" PATH_FOLDER_DELIM "%s", data_dir, (char*)g_object_get_data(G_OBJECT(widget),COUNTER_REL_FILE_PATH_KEY));
