...
static void response(SimECUGenerator *generator, char ** response, final Buffer *binResponse, final Buffer *binRequest) {
    ...
    switch(binRequest->buffer[0]) {
        case 0x02: case 0x01: {
            buffer_append(binResponse,buffer_new_cycle(ISO_15765_SINGLE_FRAME_DATA_BYTES - 2, cycle_percent[binRequest->buffer[0]][binRequest->buffer[1]]));
        } break;
        case 0x07: case 0x0A: case 0x03: {
            buffer_append(binResponse,buffer_new_cycle(ISO_15765_SINGLE_FRAME_DATA_BYTES - 1, cycle_percent[binRequest->buffer[0]][0]));                
        } break;
        case 0x04: {
        ...
        case 0x09: {
            if ( 1 < binRequest->size ) {            
                switch(binRequest->buffer[1]) {
                    ...
                    case 0x02: {
                        buffer_append(binResponse,buffer_new_cycle(17, cycle_percent[binRequest->buffer[0]][binRequest->buffer[1]]));                
                        break;
                    }
                    case 0x03: {
                    ...
                    case 0x04: {
                        buffer_append(binResponse,buffer_new_cycle(16, cycle_percent[binRequest->buffer[0]][binRequest->buffer[1]]));                
                        break;
                    }
                    case 0x05: {
                        buffer_append_byte(binResponse,0x01);
                    ...
                    case 0x06: {
                        buffer_append(binResponse,buffer_new_cycle(4, cycle_percent[binRequest->buffer[0]][binRequest->buffer[1]]));
                        break;
                    }
                    case 0x07: {
                        buffer_append_byte(binResponse,0x01);
    ...
    cycle_iterate(binRequest->buffer[0], 1 < binRequest->size ? binRequest->buffer[1] : 0, gears);
}
