...
bool record_to_json_file(char *filepath) {
    ...
    for (int i = 0; i < recorder->size; i++) {
        ad_object_Record *record = recorder->list[i];
        ...
        for(int j = 0; j < record->binResponses->size; j++) {
            ...
            if ( ecu_obj == null ) {
                ...
                cJSON_AddStringToObject(ecu_obj, "ecu", ad_buffer_to_hex_string(ecu_response->ecu->address));
            ...
            cJSON_AddStringToObject(flow_obj, "request", ad_buffer_to_hex_string(record->binRequest));

            cJSON *rs_arr = cJSON_CreateArray();
            cJSON_AddItemToObject(flow_obj, "responses", rs_arr);

            for (int k = 0; k < ecu_response->size; k++) {
                cJSON_AddItemToArray(rs_arr, cJSON_CreateString(ad_buffer_to_hex_string(ecu_response->list[k])));
            }
        }
    }
