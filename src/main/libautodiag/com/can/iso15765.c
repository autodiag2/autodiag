#include "libautodiag/com/can/iso15765.h"

Iso15765Conversation* iso15765_init_conversation(int expected_bytes) {
    Iso15765Conversation* conv = (Iso15765Conversation*)malloc(sizeof(Iso15765Conversation));
    conv->current_sn = 0;
    conv->current_data_length = 0;
    conv->remaining_data_bytes_to_receive = expected_bytes;
    conv->data = ad_buffer_new();
    ad_buffer_ensure_capacity(conv->data,expected_bytes);
    conv->data->size = conv->data->size_allocated;
    conv->ecu = null;
    return conv;
}

void iso15765_conversation_free(Iso15765Conversation *conv) {
    if ( conv != null ) {
        ad_buffer_free(conv->data);
        conv->remaining_data_bytes_to_receive = 0;
        conv->ecu = null;
        free(conv);
    }
}

bool ad_list_Iso15765Conversation_find_comparator(Iso15765Conversation *conv, ad_object_ECU* ecu) {
    return conv->ecu == ecu;
}
AD_LIST_SRC_FIND(Iso15765Conversation,
                ad_object_ECU*,ad_list_Iso15765Conversation_find_comparator
)
int Iso15765Conversation_cmp(Iso15765Conversation* e1, Iso15765Conversation* e2) {
    return e1->ecu - e2->ecu;
}
AD_LIST_SRC(Iso15765Conversation)
