...
ad_object_UDS_DTC * ad_object_UDS_DTC_new() { return AD_UDS_DTC_new(); }
...
void ad_object_UDS_DTC_free(ad_object_UDS_DTC *o) { return AD_UDS_DTC_free(o); }
...
AD_UDS_DTC * AD_UDS_DTC_new() {
    AD_UDS_DTC * dtc = (AD_UDS_DTC*)malloc(sizeof(AD_UDS_DTC));
    dtc->status = AD_UDS_DTC_STATUS_TestNotCompletedSinceLastClear | AD_UDS_DTC_STATUS_TestNotCompletedThisOperationCycle;
    ...
    dtc->to_string = AD_DTC_TO_STRING(AD_UDS_DTC_to_string);
...
AD_UDS_DTC * AD_UDS_DTC_new_from(final SAEJ1979_DTC *dtc) {
    AD_UDS_DTC * udtc = AD_UDS_DTC_new();
...
int ad_list_AD_UDS_DTC_cmp(final ad_list_AD_UDS_DTC * e1, final ad_list_AD_UDS_DTC * e2) {
    return e1 - e2;
}
AD_LIST_SRC(ad_list_AD_UDS_DTC);

static ad_list_ad_list_AD_UDS_DTC * ad_uds_read_dtcs_with_mask(final VehicleIFace * iface, final Vehicle * filter, final AD_UDS_SERVICE_READ_DTC_INFORMATION_SUB_FUNCTION sub, final byte StatusMask) {
    ad_list_ad_list_AD_UDS_DTC * result = ad_list_ad_list_AD_UDS_DTC_new();
    viface_lock(iface);
    ...
    for(int i = 0; i < iface->vehicle->ecus->size; i++) {
        ...
        final ad_list_AD_UDS_DTC * ecu_response = ad_list_AD_UDS_DTC_new();
            ...
            if ( data->buffer[0] == AD_UDS_NEGATIVE_RESPONSE ) {
                ...
                if ( (data->buffer[1] & sub) == sub ) {
                    ...
                    for(int i = 3; i < (data->size-DTC_DATA_SZ); i += (DTC_DATA_SZ+1)) {
                        final AD_UDS_DTC * dtc = AD_UDS_DTC_new();
                        memcpy(dtc->data, data->buffer + i, DTC_DATA_SZ);
                        dtc->status = data->buffer[i+DTC_DATA_SZ];
                        dtc->ecu = ecu;
                        ad_list_AD_UDS_DTC_append(ecu_response, dtc);
        ...
        ad_list_ad_list_AD_UDS_DTC_append(result, ecu_response);
    }
    viface_unlock(iface);
    return result;
}
...
ad_list_AD_UDS_DTC * ad_uds_read_all_dtcs(final VehicleIFace * iface, final Vehicle * filter) {
    ad_list_ad_list_AD_UDS_DTC * lists_dtcs = ad_uds_read_dtc_by_status_mask(iface, filter,
    ...
    ad_list_AD_UDS_DTC * dtcs = ad_list_AD_UDS_DTC_new();
...
char * ad_uds_dtc_status_to_string(AD_UDS_DTC_STATUS wanted) {
    switch (wanted) {
        case AD_UDS_DTC_STATUS_TestFailed:
            return strdup("Test Failed");
        case AD_UDS_DTC_STATUS_TestFailedThisOperationCycle:
            return strdup("Test Failed This Operation Cycle");
        case AD_UDS_DTC_STATUS_PendingDTC:
            return strdup("Pending DTC");
        case AD_UDS_DTC_STATUS_ConfirmedDTC:
            return strdup("Confirmed DTC");
        case AD_UDS_DTC_STATUS_TestNotCompletedSinceLastClear:
            return strdup("Test Not Completed Since Last Clear");
        case AD_UDS_DTC_STATUS_TestFailedSinceLastClear:
            return strdup("Test Failed Since Last Clear");
        case AD_UDS_DTC_STATUS_TestNotCompletedThisOperationCycle:
            return strdup("Test Not Completed This Operation Cycle");
        case AD_UDS_DTC_STATUS_WarningIndicatorRequested:
