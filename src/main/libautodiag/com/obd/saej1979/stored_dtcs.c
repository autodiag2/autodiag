...
char * saej1979_dtc_to_string(final DTC * dtc) {
    final Buffer * dtc_bin = ad_buffer_new();
    ad_buffer_append_bytes(dtc_bin, dtc->data, DTC_DATA_SZ);
    final byte type = (dtc_bin->buffer[0] & 0xC0) >> 6;
    dtc_bin->buffer[0] = (dtc_bin->buffer[0] & (~0xC0)) ;
    dtc_bin->size -= 1;
    char *result;
    asprintf(&result, "%c%s",iso15031_dtc_type_first_letter(type),ad_buffer_to_hex_string(dtc_bin));
    return result;
}
...
Buffer * saej1979_dtc_number(final DTC * dtc) {
    final Buffer * result = ad_buffer_new();
    ad_buffer_append_bytes(result, dtc->data, DTC_DATA_SZ);
...
char * saej1979_dtc_explanation(final DTC * dtc) {
    ...
    final char * dtc_number_str = ad_buffer_to_hex_string(dtc_number);
