...
bool testDOIP() {
    ...
    Buffer * tester_address = ad_buffer_from_ascii_hex("1234");
    {
        ...
        ad_object_DoIPMessage * msg = doip_message_diag(target_ecu, tester_address, payload);
        Buffer * serialized = doip_message_serialize(msg);
        assert(serialized != null);
        ad_object_DoIPMessage * msg2 = doip_message_parse(serialized);
    ...
    {
        ad_object_DoIPDevice * device = ad_object_DoIPDevice_new();
    ...
    }
    {
        ...
        ad_list_UDS_DTC * dtcs = uds_read_all_dtcs(iface, null);
        assert(0 < dtcs->size);
